...
uint8_t BTN_COUNT = 0;
bool uart_notif_en = false;

volatile bool ALERT_BLE_RX = false;
char bleInputString[ DEBUG_MESSAGE_MAX_LEN ];
int8_t bleInputIndex = 0;
...
void ble_cli_process(void)
{
    ...
    if(wakeup_intr_flag)
    {
        ...
        {
            cyhal_gpio_toggle((cyhal_gpio_t)PIN_USER_LED);
        }
        else if(CY_BLE_CONN_STATE_CONNECTED == Cy_BLE_GetConnectionState(app_conn_handle))
        {
            cyhal_gpio_write((cyhal_gpio_t)PIN_USER_LED, CYBSP_LED_STATE_ON);
        }
        else
        {
            cyhal_gpio_write((cyhal_gpio_t)PIN_USER_LED, CYBSP_LED_STATE_OFF);
        }
...
static void stack_event_handler(uint32_t event, void* eventParam)
{
    ...
    switch(event)
    {
        ...
        case CY_BLE_EVT_GATTS_WRITE_CMD_REQ:
        		{
                    ...
                    printf("[INFO] : GATT write characteristic with handle: 0x%x\r\n", write_req_param->handleValPair.attrHandle);

                    switch (write_req_param->handleValPair.attrHandle) {
                        case CY_BLE_LEDS_USR_LED_CHAR_HANDLE: {
                            printf("[INFO] : GATT write USR_LED characteristic with value: 0x%x\r\n", write_req_param->handleValPair.value.val[0]);
                            break;
                        }
                        case CY_BLE_UART_RXD_CHAR_HANDLE: {
                            for (int i = 0; i < write_req_param->handleValPair.value.len; i++) {
                                if (write_req_param->handleValPair.value.val[i] == '\r' || 
                                    write_req_param->handleValPair.value.val[i] == '\n') {
                                    ALERT_BLE_RX = true;
                                    printf("[INFO] : GATT write UART RX with value: %s\r\n", 
                                        bleInputString);
                                    break;
                                }

                                bleInputString[bleInputIndex] = write_req_param->handleValPair.value.val[i];
                                bleInputIndex++;
                            }

                            cy_stc_ble_gatt_handle_value_pair_t uart_tx_handle = {
                                .attrHandle = CY_BLE_UART_TXD_CHAR_HANDLE,
                                .value.val = (uint8_t *)"Recieved\r",
                                .value.len = 9,
                                .value.actualLen = 9
                            };
                            cy_stc_ble_gatts_handle_value_ntf_t uart_tx_ntf = {
                                .handleValPair = uart_tx_handle,
                                .connHandle = app_conn_handle
                            };
                            
                            if(uart_notif_en)
                            {
                                Cy_BLE_GATTS_Notification(&uart_tx_ntf);
                            }
                            break;
                        }
                    }
        ...
        case CY_BLE_EVT_GATTS_WRITE_REQ:
            {
                ...
                printf("[INFO] : GATT write characteristic with handle: 0x%x\r\n", 
                    write_req_param->handleValPair.attrHandle);
                printf("[INFO] : GATT write characteristic with len: 0x%04x\r\n", 
                    write_req_param->handleValPair.value.len);
                printf("[INFO] : GATT write characteristic with actual len: 0x%04x\r\n", 
                    write_req_param->handleValPair.value.actualLen);
                printf("[INFO] : GATT write characteristic with value: 0x%02x%02x\r\n", 
                    write_req_param->handleValPair.value.val[1], 
                    write_req_param->handleValPair.value.val[0]);

                if( CY_BLE_UART_TXD_CLIENT_CHARACTERISTIC_CONFIGURATION_DESC_HANDLE == 
                    write_req_param->handleValPair.attrHandle)
                {

                    printf("[INFO] : GATT write UART TX CCCD characteristic with value: 0x%x\r\n", 
                        write_req_param->handleValPair.value.val[0]);
                    uart_notif_en = (write_req_param->handleValPair.value.val[0] == 0x01);
                    Cy_BLE_GATTS_WriteRsp(app_conn_handle);
                }
        ...
        default:
        {
            printf("[INFO] : Unknown BLE Event 0x%lX\r\n", (unsigned long) event);
        }
    }
}
